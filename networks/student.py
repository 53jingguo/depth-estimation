...
class Equi_student(nn.Module):
    ...
    def __init__(self, num_layers, equi_h, equi_w, pretrained=False, max_depth=10.0, **kwargs):
        ...
        self.equi_dec_convs["deconv_4"] = ConvBlock(self.num_ch_dec[4], self.num_ch_dec[4])# + self.num_ch_enc[3]
        self.equi_dec_convs["upconv_4"] = ConvBlock(self.num_ch_dec[4], self.num_ch_dec[3])

        self.equi_dec_convs["deconv_3"] = ConvBlock(self.num_ch_dec[3], self.num_ch_dec[3])# + self.num_ch_enc[2]
        self.equi_dec_convs["upconv_3"] = ConvBlock(self.num_ch_dec[3], self.num_ch_dec[2])

        self.equi_dec_convs["deconv_2"] = ConvBlock(self.num_ch_dec[2], self.num_ch_dec[2])# + self.num_ch_enc[1]
        self.equi_dec_convs["upconv_2"] = ConvBlock(self.num_ch_dec[2], self.num_ch_dec[1])

        self.equi_dec_convs["deconv_1"] = ConvBlock(self.num_ch_dec[1], self.num_ch_dec[1])# + self.num_ch_enc[0]
    ...
    def forward(self, coarse_depth):

        # euqi image encoding
        if self.num_layers < 18:
            equi_enc_feat0, equi_enc_feat1, equi_enc_feat2, equi_enc_feat3, equi_enc_feat4 \
                = self.equi_encoder(coarse_depth)
        else:
            x = self.conv1(coarse_depth)
