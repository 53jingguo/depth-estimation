...
class Equi(nn.Module):
    ...
    def __init__(self, num_layers, equi_h, equi_w, pretrained=False, max_depth=10.0, **kwargs):
        ...
        self.equi_dec_convs["deconv_256"] = ConvBlock(self.num_ch_dec[0] + 32, self.num_ch_dec[0])#add
        ...
        self.conv1 = nn.Conv2d(3, 32, kernel_size=7, stride=1, padding=3,
                               bias=False, padding_mode='zeros')#add
        self.maxpool1 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)#add
    ...
    def forward(self, input_equi_image, input_cube_image):
        ...
        if self.num_layers < 18:
            ...
            x = self.conv1(input_equi_image)
            x = self.equi_encoder.relu(self.bn1(x))
            equi_enc_feat256 = x

            x = self.equi_encoder.maxpool(x)
            x = self.equi_encoder.layer0(x)
            equi_enc_feat0 = x
            x = self.maxpool1(x)
        ...
        equi_x = torch.cat([equi_x, equi_enc_feat256], 1)
        equi_x = self.equi_dec_convs["deconv_256"](equi_x)
