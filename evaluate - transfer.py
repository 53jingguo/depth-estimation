...
from networks import UniFuse, Equi, Equi_convnext, Equi_teacher, Equi_student, Equi_convnext_tea,SphFuse
import datasets
...
def main():
    ...
    with torch.no_grad():
        for batch_idx, inputs in enumerate(pbar):
            ...
            if batch_idx ==1:
                # saver.save_feature(outputs["sph_enc_feat3"][0])
                # saver.save_feature1(outputs["equi_enc_feat3"][0])
                x = np.uint8(255 * np.transpose((inputs["rgb"].cpu().numpy())[0], (1, 2, 0)))

                heatmap = np.maximum(outputs["sph_enc_feat1"][0].cpu().numpy(), 0)
                heatmap = np.mean(heatmap, axis=0)
                heatmap /= np.max(heatmap)
                heatmap = cv2.resize(heatmap, (1024, 512))
                heatmap = np.uint8(255 * heatmap)
                heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
                cv2.imwrite('src.jpg', x)
                cv2.imwrite('heatmap_sph.jpg', heatmap)
                heatmap = heatmap * 0.8 + x
                cv2.imwrite('x32_sph.jpg', heatmap)

                heatmap_bf = np.maximum(outputs["equi_enc_feat1"][0].cpu().numpy(), 0)
                heatmap_bf = np.mean(heatmap_bf, axis=0)
                heatmap_bf /= np.max(heatmap_bf)
                heatmap_bf = cv2.resize(heatmap_bf, (1024, 512))
                heatmap_bf = np.uint8(255 * heatmap_bf)
                heatmap_bf = cv2.applyColorMap(heatmap_bf, cv2.COLORMAP_JET)
                cv2.imwrite('heatmap_eq.jpg', heatmap_bf)
                heatmap_bf = heatmap_bf * 0.5 + x
                cv2.imwrite('x32_eq.jpg', heatmap_bf)
                break
