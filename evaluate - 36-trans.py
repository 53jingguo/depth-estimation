    ...
    with torch.no_grad():
        for batch_idx, inputs in enumerate(pbar):
            ...
            if batch_idx ==0:
                ...
                heatmap = np.maximum(outputs["sph_enc_feat0"][0].cpu().numpy(), 0)
                ...
                heatmap_bf = np.maximum(outputs["equi_enc_feat0"][0].cpu().numpy(), 0)
                heatmap_bf = np.mean(heatmap_bf, axis=0)
                heatmap_bf /= np.max(heatmap_bf)
                heatmap_bf = cv2.resize(heatmap_bf, (512, 256))
