...
import datasets
from networks import Equi_convnext_tea, SphFuse
...
class Trainer:
    def __init__(self, settings):
        ...
        val_file_list = fpath.format(self.settings.dataset, "test")#以前是val
        ...
        # network
        Net_dict = {"teacher": Equi_convnext_tea,
                    "sphfuse": SphFuse}
        ...
        self.model = Net(self.settings.num_layers, self.settings.height, self.settings.width,
                         self.settings.imagenet_pretrained, train_dataset.max_depth_meters,#train_dataset.max_depth_meters
        ...
        # self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer,step_size=10,gamma=0.5,last_epoch=-1)
        ...
        self.compute_loss1 = RMSELog()
        self.kldloss = torch.nn.KLDivLoss()
        ...
        # self.lut512 = read_lut(512)
        self.lut256 = read_lut(256)##360
        # self.lut128 = read_lut(128)
        self.lut64 = read_lut(64)
        self.lut32 = read_lut(32)
        self.lut16 = read_lut(16)
        # self.lut8 = read_lut(8)
        self.writers = {}
    ...
    def process_batch(self, inputs):
        ...
        # Lut512 = lut_batch(B, self.lut512)
        Lut256 = lut_batch(B, self.lut256)
        # Lut128 = lut_batch(B, self.lut128)
        Lut64 = lut_batch(B, self.lut64)
        Lut32 = lut_batch(B, self.lut32)
        Lut16 = lut_batch(B, self.lut16)
        # Lut8 = lut_batch(B, self.lut8)
        if self.train_coarse:
            outputs = self.model(equi_inputs, Lut256,Lut64,Lut32,Lut16)
            ...
            losses["depth_loss"] = self.compute_loss1(inputs["gt_depth"],
                                                   outputs["pred_depth"],
                                                   inputs["val_mask"])
    ...
    def validate(self):
        ...
        # self.model_stu.eval()
        self.model_tea.eval()
        saver = Saver('D:\\project\\UniFuse-Unidirectional-Fusion-main\\UniFuse-Unidirectional-Fusion-main\\UniFuse\\experiments_3d60_new_fuse_4\\')
        ...
        self.evaluator.print(self.epoch, 'D:\\project\\UniFuse-Unidirectional-Fusion-main\\UniFuse-Unidirectional-Fusion-main\\UniFuse\\experiments_3d60_new_fuse_4\\')
        del inputs, outputs
    ...
    def load_tea(self):
        ...
        """
        load_weights_dir = os.path.expanduser("D:\\project\\UniFuse-Unidirectional-Fusion-main\\UniFuse-Unidirectional-Fusion-main\\UniFuse\\weights_3_tea_10_3d60\\")
        #D:\\project\\UniFuse-Unidirectional-Fusion-main\\UniFuse-Unidirectional-Fusion-main\\UniFuse\\weights_3_tea_10_3d60\\
